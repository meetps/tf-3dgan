...
def generator(z, batch_size=batch_size, phase_train=True, reuse=False):
    ...
    with tf.variable_scope("gen", reuse=reuse):
        ...
        # g_5 = tf.layers.batch_normalization(g_5, training=phase_train, epsilon=1e-4, momentum=0.9)
        g_5 = batch_norm(g_5)
        g_5 = tf.nn.relu(g_5)
        print ("G5: ", g_5)
...
def trainGAN(is_dummy=False, checkpoint=None, name=model_name):
    ...
    with tf.Session(config=config) as sess:  
        ...
        if is_dummy:
            ...
            train = cifar10.train.data.reshape(-1, 32, 32, 3)
            # train = np.zeros([len(origin), img_len, img_len, img_channel])
            # for idx in range(len(origin)):
                # train[idx] = resize(origin[idx], [img_len, img_len, img_channel], mode='reflect')
